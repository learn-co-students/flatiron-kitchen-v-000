...
describe "viewing recipes" do
  context "on the show recipe page" do
    before do
    ...
    it "should show the recipe name" do
      ...
      expect(page).to have_content("Rice Pudding")
    end

    it "should show the recipe ingredients" do
      visit recipe_path(@recipe)
      expect(page).to have_content("rice")
      expect(page).to have_content("sugar")
    end

    it "should display the number of recipes that can be made with the ingredients on hand" do
      4.times { Ingredient.create(name: "rice") }
      5.times { Ingredient.create(name: "sugar") }
      visit recipe_path(@recipe)
      expect(page).to have_content("5 Rice Puddings")
    end

    it "should display the number of recipes that can be made with the ingredients on hand" do
      1.times { Ingredient.create(name: "rice") }
      10.times { Ingredient.create(name: "sugar") }
      visit recipe_path(@recipe)
      expect(page).to have_content("2 Rice Puddings")
    end
